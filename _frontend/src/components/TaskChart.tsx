 ...
 * Component: TaskChart
 *
 * Logic: This file defines a React component for displaying a list of tasks in
 * a table format. It handles tasks' due dates and provides options for editing
 * and deleting tasks.
 *
 * Used in Home page
 *
...
const TaskChart = (props) => {
    // loading state for taskData updated
    const [loading, setLoading] = useState(true);

    // Only get current day items
    const { date } = props;

    // Passing task to modal via state
    const [ taskData, useTaskData ] = useState(null);
    const [ taskToEdit, setTaskToEdit ] = useState(null);
    const [ isModalVisible, setIsModalVisible ] = useState(false);

    // userTasks
    const { userTasks, setUserTasks, updatedUserTasks, deleteUserTask, updateUserTask } = useTaskContext();
      ...
      <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        <div className="max-w-full overflow-x-auto">
          <table className="w-full table-auto">
            ...
            <tbody>
            {loading ? ( <tr><td col-span="3" >Loading... </td></tr> ) : (  userTasks?.length > 0 ? ( userTasks.map((task,index)=>(
                <tr key={task.task_id}>
